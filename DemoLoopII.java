...
public class DemoLoopII {
  public static void main(String[] args) {
    ...
    // Array
    //array name shoud be plural so able to recognize it is array instead variable
    ...
    //for-each loop
    //below is to loop every value in array "amounts"; x is a copy of each value in the array
    for (double x : amounts){
      System.out.println("for-each loop :" + x); //do reading;
    } 
    //compare to basic loop
    for (int i = 0; i < amounts.length; i++){
      System.out.println("basic for-loop :" + amounts[i]); //do reading; 
      //amounts[i] is the value itself, so it could be rewrite e.g. amounts[i]=100;
    }
    //!Sum up values (Important)
    double total = 0.0;
    for (double amount : amounts){
      //.doubleValue() -> to return double value
