...
public class DemoArraySwap {
  public static void main(String[] args) {
    ...
    for (int j = 0; j < arr1.length - 1; j++) { //-1: only need to loop 4 times not 5 times
      for (int i = 0; i < arr1.length - j - 1; i++) { //!- j - 1: don't wanna 17 replace 20: refer to Bubble sort in notes
        if (arr1[i] > arr1[i + 1]) {
          temp = arr1[i];
          arr1[i] = arr1[i + 1];
          arr1[i + 1] = temp;
    ...
    System.out.println(Arrays.toString(arr1));// inner for-loop [5, -8, 17, 2, 20] outer for-loop [-8, 2, 5, 17, 20]
